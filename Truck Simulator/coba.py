...
from utils import get_distance, get_side_job, check_combo, check_combo_addition, calc_muatan_after_addition, get_reward_df

# Initialize session state for tab selection
if "active_tab" not in st.session_state:
    st.session_state.active_tab = "Prime Trip"
...
st.sidebar.button("Prime Trip", on_click=switch_tab, args=("Prime Trip",))
st.sidebar.button("Ceperan Trip", on_click=switch_tab, args=("Ceperan Trip",))
st.sidebar.button("Go Home Trip", on_click=switch_tab, args=("Go Home Trip",))

# Display content based on the active tab
if st.session_state.active_tab == "Prime Trip":
    tab1 = True
    tab2 = False
    tab3 = False
elif st.session_state.active_tab == "Ceperan Trip":
    tab1 = False
    tab2 = True
    tab3 = False
elif st.session_state.active_tab == "Go Home Trip":
    tab1 = False
    tab2 = False
    tab3 = True
...
if tab1:
    ...
    muatan_start_input = st.number_input('Muatan (Ton): ', step=1, value=40, format='%d')
    solar_start_input = st.number_input('Solar (L): ', step=1, value=40, format='%d')
    if solar_start_input == 0:
        st.error("== NOT ENOUGH SOLAR ==")
    
    path_start = nx.shortest_path(G, source=initial_input, target=goal_input)
    ...
    if solar_start_input != 0:
        ...
        for i in route_df.index:
            ...
            path_end = nx.shortest_path(G, source=initial_end, target=initial_input)
...
if tab2:
    ...
    ceperan_goal_input = st.selectbox('Ceperan Goal', (ceperan_goals))
    ...
    st.write('#### Job 👷🏼‍♂️ yang kamu pilih: ')
